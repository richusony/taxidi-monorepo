...
export class AuthService {
  ...
  async refreshToken(token: string) {
    ...
    const newAccessToken = generateAccessToken(userExists.id, userExists.role[0]);
    if (!newAccessToken) {
      console.log('error while creating access token');
      throw new Error('Internal server error');
    }
    ...
    const addRefreshTokenToDB = authRepo.addRefreshTokenToDB(userExists.id, newRefreshToken);
